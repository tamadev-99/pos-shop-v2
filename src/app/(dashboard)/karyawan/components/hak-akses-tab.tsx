...
import { Shield, Save, CheckSquare, Info } from "lucide-react";
...
export function HakAksesTab({ initialPermissions }: HakAksesTabProps) {
  ...
  const [permissions, setPermissions] = useState<Record<string, string[]>>(() => {
    ...
    for (const role of ROLES_TO_MANAGE) {
      ...
      state[role.id] = initialPermissions?.[role.id] || getAccessiblePaths(role.id as any, {});
    }
    return state;
  });
    ...
    <Card className="border-none shadow-none bg-transparent">
      <CardHeader className="px-0 pt-0">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <CardTitle className="text-lg">Hak Akses & Modul</CardTitle>
            <CardDescription className="text-xs">
              Tentukan fitur apa saja yang dapat diakses oleh Kasir dan Manajer di toko ini.
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="px-0 space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          {ROLES_TO_MANAGE.map((role) => (
            <div key={role.id} className="rounded-xl border border-border bg-card p-4 space-y-4 shadow-sm">
        ...
        <div className="flex pt-2">
          <Button onClick={handleSave} disabled={isPending} className="w-full sm:w-auto shadow-indigo-500/20 shadow-lg px-6">
